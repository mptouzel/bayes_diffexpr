...
import time
import sys
from infer_diffexpr_lib import setup_outpath_and_logs, import_data, get_Pn1n2_s, get_sparserep, save_table, get_rhof, get_Ps,callbackFnull,callbackFdiffexpr,constr_fn,NegBinParMtr,get_Ps_pm,PoisPar,NegBinPar,get_likelihood,constr_fn_diffexpr
...
#######################################0
    ...
    freq_dtype='float64' #data type of log clone frequency axis (set to float32 when sampling synthetic human repertoires) 
    ...
    for it,dataset_pair in enumerate(((null_pair_1,null_pair_2),(test_pair_1,test_pair_2))):
        ...
        if (not loadnull and it==0) or it==1:
            ...
            prtfn('N_II/N_I='+str(NreadsII_d/NreadsI_d))
            ...
            if it==0:
                ...
                if os.path.exists(outpath+'optparas.npy'):
                    ...
                    prtfn(str(initparas))
                    callbackFnull_part=partial(callbackFnull,prtfn=prtfn)
                    outstruct = minimize(partialobjfunc, initparas, method='SLSQP', callback=callbackFnull_part, constraints=condict,tol=1e-6,options={'ftol':1e-8 ,'disp': True,'maxiter':300})
    ...
    # end computations
    endtime = time.time()
    prtfn('program elapsed:' + str(endtime - starttime))
...
if __name__ == "__main__": 
    ...
    try:#optional args
        ...
        run_index=0
        output_data_path='../Yellow_fever/prepostvaccine/'
